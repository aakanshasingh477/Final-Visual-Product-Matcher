...
#         feature_vectors = np.vstack(feature_list) if feature_list else np.array([])
...
#     input_image = uploaded_img_file if uploaded_img_file else load_image_from_url(image_url) if image_url else None
...
#                         with cols[idx % 3]:
#                             st.image(image, caption=f"Similar Image {idx + 1}", use_column_width=True)
...
# Constants
TRAINED_DB_PATH = "db"  # Make sure this folder exists in your repo with .jpg images
os.environ['SSL_CERT_FILE'] = certifi.where()
